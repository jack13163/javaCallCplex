package cplex.columngen;

/** 
  * @author:Dylan 
  * @date:2017年10月6日 下午4:22:08 
  */
import ilog.concert.*;
import ilog.cplex.*;
...
public class ColumnGen {
	...
	private class Customer extends Node {
		...
		public Customer(int external_id, double x, double y, double demand, double ready_time, double due_date,
				double service_time) {
	...
	public class MasterProblem {
		...
		public void createModel() {
			try {
				...
				for (Customer customer : all_customers.values())
					row_customers.put(customer, cplex.addRange(1, 1, "cust " + customer.id));
		...
		public void addNewColumn(Path path) {
			try {
				...
				for (Customer c : all_customers.values())
					new_column = new_column.and(cplex.column(row_customers.get(c), path.a(c)));
	...
	public class SubProblem {
			...
			public void main() {
				try {
					if (!aborted && hasIncumbent()) {
						...
						if ((getIncumbentObjValue() < Parameters.ColGen.subproblemObjVal) || (time_used > time_limit)
			...
			try {
				...
				for (int i = 0; i < all_customers.size(); i++)
					for (int j = 0; j < all_customers.size(); j++) {
						double val = all_customers.get(i).b() + all_customers.get(i).time_to_node(all_customers.get(j))
								+ all_customers.get(i).time_at_node() - all_customers.get(j).a();
						if (M < val)
							M = val;
					}
		...
		public void savePath(int nSol) {
			try {
				// print
				/*
				 * for (int i : all_nodes.keySet()) for (int j : all_nodes.keySet()) if
				 * (cplex.getValue(x[i][j],nSol) > 0.99999) System.out.println(i+","+j);
				 */
				// save
	...
	private void ReadData() {
		try {
			...
			String strsql = "SELECT Customer, Xcoord, Ycoord, Demand, ReadyTime, DueDate, ServiceTime FROM Customers WHERE Instance='"
					+ instance + "';";
